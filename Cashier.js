	...
	async ledger_update(kblock, limit) {
		...
		if (chunk.mblocks.length === 0) {
			...
			if((total_pos_stake > 0) && (sblocks.length > 0)){
				let cont = new ContractMachine.Contract();
		...
		for(let i = 0; i < chunk.txs.length; i++){
			...
			if (ContractMachine.isContract(tx.data)) {
				try {
					...
					contracts[tx.hash] = await ContractMachine.processData(_tx, this.db, kblock);
	...
	async cashier(run_once) {
        try {
			...
			if (next) {
				...
				await this.ledger_update(block, this.config.cashier_chunk_size);
			} else {
